 ...
 */
@Component
public class DataLogBatchQueue<T> {
...
//			insert data log
            LogUtils.insertDataLog(template, (List<DataLog>) dataList);
        };
	}
...
//		init buffer / timeout
		if(buffer==-1) buffer=MapUtil.getInt(Data.attr, "data_log_buffer_size", 100);
		if(timeout==-1) timeout=MapUtil.getInt(Data.attr, "data_log_timeout", 100);
//		add queue
